 ...
 (deftest element-creation-test
   (testing "Creating contact elements"
     (let [contact (converter/create-contact "X1" false {:x 1 :y 1})
       ...
       (is (= "contact" (:type contact)))
       (is (= "contact_negated" (:type negated-contact)))
   ...
   (testing "Creating coil elements"
     (let [coil (converter/create-coil "Y1" false {:x 10 :y 1})
       ...
       (is (= "coil" (:type coil)))
       (is (= "coil_negated" (:type negated-coil)))
 ...
 (deftest il-to-ld-conversion-test
   ...
   (testing "Converting ST instructions"
     (let [instructions [(parser/->ILInstruction "LD" "X1" nil)
       ...
       (is (= "contact" (get-in ld-network [:elements 0 :type])))
       (is (= "coil" (get-in ld-network [:elements 1 :type])))
       (is (= "X1" (get-in ld-network [:elements 0 :properties :variable])))
   ...
   (testing "Converting AND and OR instructions"
     (let [instructions [(parser/->ILInstruction "LD" "X1" nil)
      ...
      ;; Check correct element types
       (is (= 2 (count (filter #(= (:type %) "contact") (:elements ld-network)))))
       (is (= 1 (count (filter #(= (:type %) "and") (:elements ld-network)))))
       (is (= 1 (count (filter #(= (:type %) "or") (:elements ld-network)))))
       (is (= 1 (count (filter #(= (:type %) "coil") (:elements ld-network)))))))
   ...
   (testing "Converting negated instructions"
     (let [instructions [(parser/->ILInstruction "LDN" "X1" nil)
      ...
      ;; Check correct element types
       (is (= 1 (count (filter #(= (:type %) "contact_negated") (:elements ld-network)))))
       (is (= 1 (count (filter #(= (:type %) "coil_negated") (:elements ld-network)))))))
   ...
   (testing "Converting timer instructions"
     (let [instructions [(parser/->ILInstruction "LD" "X1" nil)
      ...
      ;; Check timer element
       (let [timer-element (first (filter #(= (:type %) "timer_on") (:elements ld-network)))]
 ...
 (deftest network-modification-test
   (testing "Adding and removing elements"
     (let [contact (converter/create-contact "X1" false {:x 1 :y 1})
       ...
       (is (= "coil" (get-in network-without-contact [:elements 0 :type])))))
